...
export const imageUrl = (date: string, n: number) => `${publicBaseUrl()}/api/scemalta/img/${date}/${n}.jpg`
...
// ── Autorização das rotas: cron (Bearer CRON_SECRET) ou usuário do dashboard ─
...
export async function authorize(req: NextRequest): Promise<Caller> {
  ...
  if (!user) return null
  return user.role === 'admin' || (user.permissions as string[]).includes('noticias') ? 'user' : null
}
