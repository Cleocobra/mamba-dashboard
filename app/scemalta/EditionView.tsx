  ...
  return (
    <article className="space-y-10">
      <header className="space-y-3">
        <p className="text-xs font-black uppercase tracking-[0.2em] text-[#22C55E]">{formatLongDate(e.date)}</p>
        <h1 className="text-3xl font-black leading-tight tracking-tight md:text-4xl">{e.manchete}</h1>
        {e.instagram?.permalink && (
          <a href={e.instagram.permalink} target="_blank" rel="noreferrer" className="inline-block text-sm font-semibold text-[#22C55E] hover:underline">
            Ver no Instagram →
          </a>
        )}
      </header>
      ...
      <div className="-mx-4 flex snap-x gap-3 overflow-x-auto px-4 pb-2">
        {Array.from({ length: cards }, (_, n) => (
          ...
          <img
            key={n}
            src={`/api/scemalta/img/${e.date}/${n}.jpg?v=${encodeURIComponent(e.updatedAt)}`}
      ...
      <section className="space-y-6">
        {e.noticias.map((n, i) => (
          <div key={i} className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
            <p className="text-[11px] font-black uppercase tracking-[0.2em] text-[#22C55E]">{String(i + 1).padStart(2, '0')} · {n.categoria}</p>
            <h2 className="mt-2 text-xl font-black leading-snug">{n.titulo}</h2>
            <p className="mt-2 leading-relaxed text-slate-300">{n.resumo}</p>
            <a href={n.link} target="_blank" rel="noreferrer nofollow" className="mt-3 inline-block text-sm font-semibold text-slate-400 hover:text-[#22C55E]">
      ...
      {e.oportunidades.length > 0 && (
        <section className="rounded-2xl border border-[#22C55E]/30 bg-[#22C55E]/5 p-5">
          <h2 className="text-sm font-black uppercase tracking-[0.2em] text-[#22C55E]">Oportunidades para quem empreende em SC</h2>
          <ul className="mt-4 space-y-4">
            {e.oportunidades.map((o, i) => (
              <li key={i}>
                ...
                <a href={o.link} target="_blank" rel="noreferrer nofollow" className="mt-1 inline-block text-xs font-semibold text-slate-400 hover:text-[#22C55E]">
                  {o.fonte} →
                </a>
              </li>
            ))}
