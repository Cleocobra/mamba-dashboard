...
import { getCashflowPorDias } from '@/lib/li-client'
...
export default function FluxoPage() {
  ...
  const fetchCashflow = useCallback(async () => {
    try {
      const { cashflow: cf, total_entradas } = await getCashflowPorDias(dias)
      setCashflow(cf)
      setSummary({ total_entradas, total_saidas: 0, saldo_periodo: total_entradas })
    } catch (err) {
      console.error(err)
    }
  }, [dias])
  ...
  const manual_entradas = entries.filter(e => e.tipo === 'entrada').reduce((a, e) => a + e.valor, 0)
  const manual_saidas   = entries.filter(e => e.tipo === 'saida').reduce((a, e) => a + e.valor, 0)

  return (
    <div className="flex h-screen bg-mamba-black overflow-hidden">
      <Sidebar />
      <div className="flex-1 flex flex-col overflow-hidden ml-64">
        <Header title="Fluxo de Caixa" subtitle="Entradas, saídas e saldo do período" onRefresh={handleRefresh} isRefreshing={isRefreshing} />

        <main className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
          {/* Metrics */}
          <div className="grid grid-cols-3 gap-4">
            <MetricCard label="Total Entradas" value={summary.total_entradas + manual_entradas} isCurrency icon={TrendingUp}  accentColor="green" />
            <MetricCard label="Total Saídas"   value={summary.total_saidas  + manual_saidas}   isCurrency icon={TrendingDown} accentColor="red"   />
            <MetricCard label="Saldo"          value={summary.saldo_periodo + manual_entradas - manual_saidas} isCurrency icon={Wallet} accentColor="gold" />
          </div>
          ...
          <div className="rounded-xl border border-mamba-border bg-mamba-card p-5">
              ...
              <div className="mb-4 p-4 rounded-xl bg-mamba-dark border border-mamba-border space-y-3 animate-fade-in-up">
                <div className="flex items-center gap-2">
                  {(['entrada', 'saida'] as const).map(t => (
                    <button key={t} onClick={() => setForm(f => ({ ...f, tipo: t, categoria: t === 'entrada' ? 'Vendas' : 'Outros' }))}
                      className={`flex items-center gap-2 px-3 py-2 text-xs font-semibold rounded-lg border transition-all cursor-pointer ${
                        form.tipo === t
                          ? t === 'entrada' ? 'bg-green-400/10 border-green-400/30 text-green-400' : 'bg-red-400/10 border-red-400/30 text-red-400'
                          : 'border-mamba-border text-mamba-silver hover:border-mamba-silver'
                      }`}
                    >
                      {t === 'entrada' ? <ArrowUpCircle className="w-3.5 h-3.5" /> : <ArrowDownCircle className="w-3.5 h-3.5" />}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="text" placeholder="Ex: Fornecedor X" value={form.descricao}
            ...
            {entries.length === 0 ? (
              ...
              <div className="space-y-2">
                {entries.map(entry => (
                  <div key={entry.id}
                    className="flex items-center gap-3 p-3 rounded-lg bg-mamba-dark border border-mamba-border">
                    <div className={`flex items-center justify-center w-8 h-8 rounded-lg flex-shrink-0 ${entry.tipo === 'entrada' ? 'bg-green-400/10' : 'bg-red-400/10'}`}>
                      {entry.tipo === 'entrada'
                    ...
                    <span className={`text-sm font-bold tabular-nums flex-shrink-0 ${entry.tipo === 'entrada' ? 'text-green-400' : 'text-red-400'}`}>
