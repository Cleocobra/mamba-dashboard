...
export default function NoticiasPage() {
  ...
  const patch = (date: string, body: Partial<Edition>, label: string) =>
    call(label, `/api/editions/${date}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
    ...
    <div className="min-h-screen bg-ui-black">
      <div className="max-w-6xl mx-auto">
        <header className="sticky top-0 z-20 flex items-center gap-3 px-4 md:px-6 py-3.5 bg-ui-black/80 backdrop-blur-md border-b border-ui-border">
          <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-ui-accent text-base font-black text-ui-black">↗</span>
          <div className="flex-1 min-w-0">
            <h1 className="text-base md:text-lg font-black tracking-tight text-ui-white truncate">SC em Alta · Painel</h1>
            <p className="text-xs text-ui-silver mt-0.5 truncate">Edição diária: gerar, revisar e publicar no site e no Instagram</p>
          </div>
        ...
        <main className="px-4 py-4 md:px-6 md:py-6 space-y-5">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button onClick={() => call('Gerar edição de hoje', '/api/run')} disabled={busy !== null}
            ...
            {config && (
              <span className="text-[11px] text-ui-silver/60 ml-auto">
                {config.autoPublish ? 'O agendador publica às 8h, exceto edições rejeitadas.' : 'O agendador só publica edições aprovadas.'}
              </span>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-5">
            ...
            <div className="bg-ui-card border border-ui-border rounded-xl overflow-hidden">
              <p className="px-4 py-3 text-[10px] font-bold tracking-[0.2em] text-ui-silver/50 uppercase border-b border-ui-border">Edições</p>
            ...
            <div className="space-y-5">
              {!current ? (
                <div className="bg-ui-card border border-ui-border rounded-xl p-8 text-center text-sm text-ui-silver">Selecione uma edição.</div>
              ) : (
                <>
                  <div className="bg-ui-card border border-ui-border rounded-xl p-5 space-y-4">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="text-xs text-ui-silver">{current.rawCount} itens coletados · {current.noticias.length} notícias · {current.oportunidades.length} oportunidades · {cards} cards</span>
                    ...
                    {current.noticias.length > 0 && (
                      <>
                        <div className="space-y-2">
                          <label className="text-[10px] font-bold tracking-[0.2em] text-ui-silver/50 uppercase">Manchete</label>
                          <input value={manchete} onChange={e => setManchete(e.target.value)} disabled={!editable} maxLength={80}
                            className="w-full bg-ui-dark border border-ui-border rounded-lg px-3 py-2 text-sm text-ui-white disabled:opacity-60" />
                        </div>
                        <div className="space-y-2">
                          <label className="text-[10px] font-bold tracking-[0.2em] text-ui-silver/50 uppercase">Legenda do Instagram</label>
                          <textarea value={legenda} onChange={e => setLegenda(e.target.value)} disabled={!editable} rows={7}
                            className="w-full bg-ui-dark border border-ui-border rounded-lg px-3 py-2 text-sm text-ui-white disabled:opacity-60 font-sans" />
                  ...
                  {current.noticias.length > 0 && (
                    <div className="bg-ui-card border border-ui-border rounded-xl p-5 space-y-3">
                      <p className="text-[10px] font-bold tracking-[0.2em] text-ui-silver/50 uppercase">Cards do carrossel</p>
                  ...
                  {current.noticias.length > 0 && (
                    <div className="bg-ui-card border border-ui-border rounded-xl p-5 space-y-4">
                      <p className="text-[10px] font-bold tracking-[0.2em] text-ui-silver/50 uppercase">Notícias</p>
                      {current.noticias.map((n, i) => (
                        <div key={i} className="border-b border-ui-border/60 pb-3 last:border-0 last:pb-0">
                          <p className="text-[10px] font-bold uppercase tracking-wider text-ui-accent">{i + 1} · {n.categoria}</p>
                          <p className="text-sm font-bold text-ui-white mt-1">{n.titulo}</p>
                          <p className="text-xs text-ui-silver mt-1">{n.resumo}</p>
                          <a href={n.link} target="_blank" rel="noreferrer" className="text-[11px] text-ui-silver/60 hover:text-ui-accent">{n.fonte} ↗</a>
                        </div>
                      ))}
                        ...
                        <>
                          ...
                          {current.oportunidades.map((o, i) => (
                            <div key={i} className="border-b border-ui-border/60 pb-3 last:border-0 last:pb-0">
                              ...
                              <p className="text-sm font-bold text-ui-white mt-1">{o.titulo}</p>
                              <p className="text-xs text-ui-silver mt-1">{o.descricao}</p>
                              <a href={o.link} target="_blank" rel="noreferrer" className="text-[11px] text-ui-silver/60 hover:text-green-400">{o.fonte} ↗</a>
                            </div>
                          ))}
                        </>
                      )}
