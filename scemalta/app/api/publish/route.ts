...
import { authorize, publishEdition, todaySP, isValidDate } from '@/lib/pipeline'

export const dynamic = 'force-dynamic'
export const maxDuration = 300

// Publica a edição (renderiza os cards, libera no site e posta no Instagram).
// Cron: POST /api/publish  (Authorization: Bearer CRON_SECRET) → publica a de hoje,
// a menos que tenha sido rejeitada no painel (ou, com SCEMALTA_AUTO_PUBLISH=false, não aprovada).
// Painel: POST /api/publish?date=YYYY-MM-DD → publica sempre.
